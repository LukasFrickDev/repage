...
def _persist_result(delivery_id, *, success, error_code=None):
    now = timezone.now()
    with transaction.atomic():
...
def process_delivery(delivery_id):
    ...
    try:
        ...
        return _persist_result(delivery.id, success=False, error_code=code)
    logger.info(
        'email_delivery_sent',
        extra={
            'lead_id': str(delivery.lead_id),
    ...
    return _persist_result(delivery.id, success=True)
