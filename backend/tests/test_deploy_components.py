...
def run_plan(cwd: Path, state_status: str, state_sha: str, marker: int, target: str) -> tuple[int, int, int, int]:
    result = subprocess.run(
        ['bash', str(PLAN_HELPER), state_status, state_sha, str(marker), target],
        cwd=cwd,
        capture_output=True,
        text=True,
        check=True,
